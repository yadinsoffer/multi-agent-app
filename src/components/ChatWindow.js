...
const yadinCallMessage = { 
    ...
    text: "Incoming call from Yadin...", 
    isIncomingCall: true, 
    isActiveCall: false 
};

const senderColors = {
    "Lance (Synthetic Teams)": "#FFFFFF",
    "John (Synthetic Teams)": "#FFFFFF",
    "Sydney (Synthetic Teams)": "#FFFFFF",
    "Ron (Synthetic Teams)": "#FFFFFF",
    "Lorenzo Larini": "#FFFFFF",
};

const roleColors = {
    "Team Lead": { bg: "#F3F0FF", text: "#6B46C1" },      // Purple theme
    "Voice Coordinator": { bg: "#E0F2FE", text: "#0369A1" }, // Blue theme
    "Sales Specialist": { bg: "#FEF3C7", text: "#B45309" },  // Orange theme
    "Listing Specialist": { bg: "#ECFDF5", text: "#047857" }  // Green theme
};

const getRoleFromSender = (sender) => {
    if (sender.includes("Lance")) return "Team Lead";
    if (sender.includes("Sydney")) return "Voice Coordinator";
    if (sender.includes("John")) return "Sales Specialist";
    if (sender.includes("Ron")) return "Listing Specialist";
    if (sender.includes("Jerry")) return "Listing Specialist";
    return null;
};

const listingTasks = [
    "Gathering property details and specifications",
    "Preparing professional photography schedule",
    "Creating property description draft",
    "Setting up virtual tour arrangements",
    "Organizing staging consultation",
    "Preparing comparative market analysis",
    "Setting up MLS listing draft",
    ...
    useEffect(() => {
        if (showRonMessage) {
            const timer = setTimeout(() => {
                ...
                listingTasks.forEach((task, index) => {
